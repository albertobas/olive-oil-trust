  ...
  context('succeeds', function () {
    it('depositing ether', async function () {
      await expect(
        ...
        .to.emit(this.contracts.commercialUnitsEscrow, EventsEscrow.EtherDeposited)
    ...
    it('setting state, buyer address, buyer wallet address and price', async function () {
      await this.contracts.commercialUnitsEscrow
        .connect(this.signers.acc2)
        .makePayment(escrowId, this.signers.acc3.address, options);
      const escrow = await this.contracts.commercialUnitsEscrow.escrow(escrowId);
